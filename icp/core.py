...
def icp(source, destination, dim_offset='all', dim_scale='all', num_iter=100, use_gpu=True, disable_tqdm=False):
    ...
    update_scl_new = tf.compat.v1.scatter_update(scl_new, dim_scl, tf.tile((tf.reduce_sum(src_gth * tf.gather(dst_cnt, dim_scl, axis=1), keepdims=True)
                                                                  / tf.reduce_sum(src_gth * (src_gth - tf.gather(src_mean, dim_scl, axis=1)),
                                                                                  keepdims=True))[:, 0], [len(dim_scale)]))
    update_off_new = tf.compat.v1.scatter_update(off_new, dim_off, tf.gather(dst_mean[0, :], dim_offset)
                                       - tf.gather(scl_new, dim_offset) * tf.gather(src_mean[0, :], dim_offset))
    update_scl = tf.compat.v1.assign(scl, scl * scl_new)
    update_off = tf.compat.v1.assign(off, scl_new * off + off_new)
    update_src_scl = tf.compat.v1.assign(src, src * tf.expand_dims(scl_new, 0))
    update_src_off = tf.compat.v1.assign(src, src + tf.expand_dims(off_new, 0))
